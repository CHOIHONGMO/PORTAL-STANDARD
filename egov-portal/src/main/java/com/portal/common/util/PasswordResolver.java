...
public class PasswordResolver {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PasswordResolver.class);
    ...
    private boolean isCryptoEnabled() {
        try {
            ...
            try {
                Method method = cryptoService.getClass().getMethod("isCrypto");
                ...
                try {
                    Method getConfigMethod = cryptoService.getClass().getMethod("getCryptoConfig");
                    Object cryptoConfig = getConfigMethod.invoke(cryptoService);

                    if (cryptoConfig != null) {
                        Method isCryptoMethod = cryptoConfig.getClass().getMethod("isCrypto");
                 ...
                 // 26.03.06 KISA 보안취약점 조치 : 구체적 Exception 추가
                } catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | ClassCastException e2) {
         ...
         // 26.03.06 KISA 보안취약점 조치 : 구체적 Exception 추가
        } catch (SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | ClassCastException e) {
    ...
    public String resolvePassword(String propertyKey) {
        try {
            ...
            String passwordValue = PortalProperties.getProperty(propertyKey);
            ...
            if (cryptoEnabled) {
                ...
                } catch (RuntimeException e) { // 26.03.06 KISA 보안취약점 조치 : 구체적 Exception 추가
                    LOGGER.error("패스워드 복호화 실패: {} - {}", propertyKey, e.getMessage());
            ...
            LOGGER.error("패스워드 해결 중 오류 발생: {} - {}", propertyKey, e.getMessage());
            // 오류 발생 시 원본 값 반환
            return PortalProperties.getProperty(propertyKey);
        }
    }
