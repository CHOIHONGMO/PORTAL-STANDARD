...
const BoardMstrList: React.FC = () => {
  ...
  const fetchList = async (page = 1) => {
    try {
      setLoading(true);
      const url = `/cop/bbs/SelectBBSMasterInfs.api?pageIndex=${page}&searchCnd=${searchCnd}&searchWrd=${encodeURIComponent(searchWrd)}`;
      ...
      console.error('게시판 마스터 목록 조회 실패', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container p_main">
      <div className="page-header">
        <h2>게시판 생성관리</h2>
        <p style={{ color: 'var(--text-muted)', marginTop: '8px' }}>시스템 내 모든 게시판 마스터 정보 및 속성을 조회하고 편집합니다.</p>
      </div>
      ...
      <div className="list-utils" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div className="total-count" style={{ fontSize: '0.95rem' }}>
          게시판 개수 : <strong>{totalCount}</strong>개
        </div>
        <Link
          to="/admin/board/new"
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>가입된 게시판 생성 정보가 없습니다.</td>
              </tr>
            ) : (
              resultList.map((item, idx) => (
                <tr key={item.bbsId}>
                  ...
                  <td>
                    <Link to={`/admin/board/${item.bbsId}`} style={{ color: 'var(--primary-color)', fontWeight: 600 }}>
                      {item.bbsNm}
                    </Link>
                  </td>
                  <td>{item.bbsTyCodeNm}</td>
                  <td>{item.bbsAttrbCodeNm}</td>
