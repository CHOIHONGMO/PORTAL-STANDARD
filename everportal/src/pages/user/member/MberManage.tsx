...
const MberManage: React.FC = () => {
  ...
  const fetchMemberList = async (page = 1) => {
    try {
      setLoading(true);
      const url = `/user/member/mber/selectMberList.api?pageIndex=${page}&sbscrbSttus=${sbscrbSttus}&searchCondition=${searchCondition}&searchKeyword=${encodeURIComponent(searchKeyword)}`;
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('선택된 회원을 삭제하시겠습니까?')) {
      try {
        ...
        const response: any = await apiClient.post('/user/member/mber/deleteMber.api', { checkedIdForDel });
        if (response.resultCode === 'SUCCESS') {
          alert('성공적으로 삭제되었습니다.');
