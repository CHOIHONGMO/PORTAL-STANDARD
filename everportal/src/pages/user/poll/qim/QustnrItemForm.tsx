...
const QustnrItemForm = () => {
  ...
  const initForm = async () => {
    try {
      setLoading(true);
      if (isEdit) {
        const detailRes: any = await apiClient.get(`/user/poll/qim/selectQustnrItemDetail.api?qustnrIemId=${id}`);
        ...
        if (paramQestnrId) {
          const qustnrRes: any = await apiClient.get(`/user/poll/qmc/selectQustnrDetail.api?qestnrId=${paramQestnrId}`);
        ...
        if (paramQesitmId) {
          const qestnRes: any = await apiClient.get(`/user/poll/qqm/selectQustnrQestnDetail.api?qestnrQesitmId=${paramQesitmId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? '/user/poll/qim/updateQustnrItem.api' : '/user/poll/qim/insertQustnrItem.api';
