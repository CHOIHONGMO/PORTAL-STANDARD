...
const QustnrRespondForm = () => {
  ...
  const loadSurveyDetail = async () => {
    try {
      setLoading(true);
      // 1. 공통코드 로드
      const sexRes: any = await apiClient.get('/user/poll/cmm/selectCmmCode.api?codeId=COM014');
      if (sexRes.resultCode === 'SUCCESS') setSexCodes(sexRes.resultList || []);

      const occpRes: any = await apiClient.get('/user/poll/cmm/selectCmmCode.api?codeId=COM034');
      if (occpRes.resultCode === 'SUCCESS') setOccpCodes(occpRes.resultList || []);

      // 2. 설문 문항 및 항목 정보 로드
      const detailRes: any = await apiClient.get(`/user/poll/qnn/selectQustnrRespondDetail.api?qestnrId=${id}&qestnrTmplatId=${qestnrTmplatId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response: any = await apiClient.post('/user/poll/qnn/insertQustnrRespond.api', payload);
