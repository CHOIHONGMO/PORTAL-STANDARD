...
const QustnrRespondManageForm = () => {
  ...
  const fetchCodes = async () => {
    try {
      const [sexRes, jobRes]: any[] = await Promise.all([
        apiClient.get('/user/poll/cmm/selectCmmCode.api?codeId=COM014'),
        apiClient.get('/user/poll/cmm/selectCmmCode.api?codeId=COM034')
      ]);
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const response: any = await apiClient.get(`/user/poll/qrm/selectQustnrRespondManageDetail.api?qestnrRespondId=${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let url = '/user/poll/qrm/insertQustnrRespondManage.api';
      if (isEdit) {
        url = '/user/poll/qrm/updateQustnrRespondManage.api';
