...
const MOCK_APPLICATIONS: ServiceApplyItem[] = [
  { id: 3, applyDate: '2026-06-13', title: '표준프레임워크 인증서 발급', count: 300, status: '발급대기', agency: '행정안전부', contact: '1544-5555' },
...
const ServiceManage = () => {
  ...
  const [items, setItems] = useState<ServiceApplyItem[]>(MOCK_APPLICATIONS);
  const [page, setPage] = useState(1);

  const handleSearch = () => {
    // Local date filtering logic
  ...
  const handleApplyClick = (title: string) => {
    alert(`"${title}"에 대한 추가 민원 신청이 완료되었습니다.`);
  };
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
      ...
      <div className="location" style={{ marginBottom: '20px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
        ...
        <span style={{ marginLeft: '6px', fontWeight: 600, color: 'var(--text-main)' }}>민원신청</span>
      </div>
