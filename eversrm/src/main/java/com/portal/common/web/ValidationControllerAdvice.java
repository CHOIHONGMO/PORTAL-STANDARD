...
public class ValidationControllerAdvice {
	...
	@Autowired
	private PortalMessageSource egovMessageSource;
	...
	private String getAnnotationMessage(ConstraintViolation<Object> violation) {
		try {
			...
			Method messageMethod = annotationClass.getMethod("message");
			...
			// 사용자가 명시적으로 지정한 메시지
			return message;
		} catch (NoSuchMethodException | SecurityException | IllegalAccessException | InvocationTargetException e) { // 26.03.06 KISA 보안취약점 조치 : 예외처리 구체화
			return null;
		}
	}
	...
	private String replaceMessageKeys(String message) {
		...
		while (matcher.find()) {
			...
			} catch (NoSuchMessageException e) { // 26.03.06 KISA 보안취약점 조치 : 예외처리 구체화
	...
	private Integer getAnnotationValue(ConstraintViolation<Object> violation, String attributeName) {
		...
		} catch (NullPointerException | ClassCastException e) { // 26.03.06 KISA 보안취약점 조치 : 예외처리 구체화
			LOGGER.debug("어노테이션 속성값 조회 중 오류 발생: {}", e.getMessage());
		}
		return null;
	}
	...
	private String getValidationMessage(String messageKey, String defaultMessage) {
		try {
			return egovMessageSource.getReloadableResourceBundleMessageSource().getMessage(messageKey, null, defaultMessage,
					Locale.getDefault());
		} catch (NullPointerException e) { // 26.03.06 KISA 보안취약점 조치 : 예외처리 구체화
			return defaultMessage;
		}
	}
