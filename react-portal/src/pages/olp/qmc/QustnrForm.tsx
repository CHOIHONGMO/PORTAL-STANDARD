...
const QustnrForm = () => {
  ...
  const initForm = async () => {
    try {
      setLoading(true);
      // 1. 공통코드 가져오기
      const codeRes: any = await apiClient.get('/uss/olp/cmm/selectCmmCode.api?codeId=COM034');
      if (codeRes.resultCode === 'SUCCESS') {
        setTargetCodes(codeRes.resultList || []);
      }
      // 2. 템플릿 목록 가져오기
      const tmplatRes: any = await apiClient.get('/uss/olp/qmc/selectQustnrTmplatList.api');
      ...
      // 3. 수정 모드인 경우 상세 로드
      if (isEdit) {
        const detailRes: any = await apiClient.get(`/uss/olp/qmc/selectQustnrDetail.api?qestnrId=${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? '/uss/olp/qmc/updateQustnr.api' : '/uss/olp/qmc/insertQustnr.api';
