...
const QustnrQestnForm = () => {
  ...
  const initForm = async () => {
    try {
      setLoading(true);
      // 1. 공통코드 가져오기
      const codeRes: any = await apiClient.get('/uss/olp/cmm/selectCmmCode.api?codeId=COM018');
      ...
      // 2. 수정모드 로드
      if (isEdit) {
        const detailRes: any = await apiClient.get(`/uss/olp/qqm/selectQustnrQestnDetail.api?qestnrQesitmId=${id}`);
        ...
        if (paramId) {
          const qustnrDetailRes: any = await apiClient.get(`/uss/olp/qmc/selectQustnrDetail.api?qestnrId=${paramId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? '/uss/olp/qqm/updateQustnrQestn.api' : '/uss/olp/qqm/insertQustnrQestn.api';
