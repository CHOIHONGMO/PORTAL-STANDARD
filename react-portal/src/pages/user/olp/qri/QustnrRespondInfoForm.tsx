...
import QustnrListPopup from '@/pages/user/olp/qmc/QustnrListPopup';
import QustnrQestnListPopup from '@/pages/user/olp/qqm/QustnrQestnListPopup';
...
const QustnrRespondInfoForm = () => {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const response: any = await apiClient.get(`/uss/olp/qri/selectQustnrRespondInfoDetail.api?qestnrQesrspnsId=${id}`);
  ...
  const fetchItemList = async () => {
    try {
      const response: any = await apiClient.get(`/uss/olp/qim/selectQustnrItemList.api?searchMode=Y&qestnrQesitmId=${qestnrQesitmId}`);
  ...
  const fetchQestnDetail = async (qesitmId: string) => {
    try {
      const response: any = await apiClient.get(`/uss/olp/qqm/selectQustnrQestnDetail.api?qestnrQesitmId=${qesitmId}`);
      if (response.resultCode === 'SUCCESS' && response.result) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let url = '/uss/olp/qri/insertQustnrRespondInfo.api';
      if (isEdit) {
        url = '/uss/olp/qri/updateQustnrRespondInfo.api';
