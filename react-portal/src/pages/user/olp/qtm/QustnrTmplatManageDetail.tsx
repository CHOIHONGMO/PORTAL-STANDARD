...
const QustnrTmplatManageDetail = () => {
  ...
  const fetchTmplatDetail = async () => {
    try {
      setLoading(true);
      const response: any = await apiClient.get(`/uss/olp/qtm/selectQustnrTmplatManageDetail.api?qestnrTmplatId=${id}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response: any = await apiClient.post('/uss/olp/qtm/deleteQustnrTmplatManage.api', { qestnrTmplatId: id });
      if (response.resultCode === 'SUCCESS') {
        alert('성공적으로 삭제되었습니다.');
