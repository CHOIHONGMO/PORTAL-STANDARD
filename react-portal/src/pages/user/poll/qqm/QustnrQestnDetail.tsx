...
const QustnrQestnDetail = () => {
  ...
  const fetchQestnDetail = async () => {
    try {
      setLoading(true);
      const response: any = await apiClient.get(`/user/poll/qqm/selectQustnrQestnDetail.api?qestnrQesitmId=${id}`);
  ...
  const handleDelete = async () => {
    if (window.confirm('정말 삭제하시겠습니까?')) {
      try {
        const response: any = await apiClient.post('/user/poll/qqm/deleteQustnrQestn.api', {
          qestnrQesitmId: id,
        });
        if (response.resultCode === 'SUCCESS') {
          alert('삭제가 완료되었습니다.');
