...
import EgovIdDplctCnfirm from '@/pages/user/umt/EgovIdDplctCnfirm';
...
const EgovMberSbscrb: React.FC = () => {
  ...
  const fetchCommonCodes = async () => {
    try {
      const response: any = await apiClient.get('/cmm/code/selectMemberCodeDetails.api');
      if (response.resultCode === 'SUCCESS') {
        const { passwordHintList, sexdstnCodeList } = response.result;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response: any = await apiClient.post('/uss/umt/cmm/insertMberSbscrb.api', payload);
      if (response.resultCode === 'SUCCESS') {
        alert('회원가입이 완료되었습니다. 로그인해 주세요.');
        navigate('/login');
      } else {
        alert(response.resultMessage || '회원가입 실패');
      }
    } catch (error) {
      console.error('회원 가입 처리 실패', error);
  ...
  return (
    <div className="container p_main" style={{ maxWidth: '800px', margin: '40px auto' }}>
      <div className="page-header" style={{ textAlign: 'center' }}>
        <h2>일반 회원가입</h2>
        <p style={{ color: 'var(--text-muted)', marginTop: '8px' }}>포털서비스 회원 가입을 위한 세부 정보를 입력해주세요.</p>
      </div>
      ...
      <div className="glass" style={{ padding: '40px', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', backgroundColor: 'var(--surface-color)' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <input
                ...
                value={mberId}
                readOnly
                placeholder="아이디 중복 검색 버튼을 클릭하세요."
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginTop: '20px' }}>
            <button type="submit" className="btn-submit" style={{ width: '160px', padding: '14px', cursor: 'pointer' }}>가입신청</button>
            <button
              type="button"
              className="btn-home"
              onClick={() => navigate('/')}
