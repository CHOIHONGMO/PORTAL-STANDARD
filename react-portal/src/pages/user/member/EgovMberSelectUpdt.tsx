...
const EgovMberSelectUpdt: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const codeResponse: any = await apiClient.get('/common/code/selectMemberCodeDetails.api');
      ...
      // Fetch member detail
      const memberResponse: any = await apiClient.get(`/user/member/mber/selectMber.api?selectedId=${id}`);
  ...
  const handleDelete = async () => {
    if (window.confirm('정말 삭제하시겠습니까?')) {
      try {
        const response: any = await apiClient.post('/user/member/mber/deleteMber.api', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response: any = await apiClient.post('/user/member/mber/updateMber.api', payload);
      if (response.resultCode === 'SUCCESS') {
