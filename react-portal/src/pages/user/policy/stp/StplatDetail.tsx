...
const StplatDetail = () => {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const response: any = await apiClient.get(`/user/policy/stp/selectStplatDetail.api?useStplatId=${id}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response: any = await apiClient.post('/user/policy/stp/deleteStplatCn.api', { useStplatId: id });
      if (response.resultCode === 'SUCCESS') {
        alert('성공적으로 삭제되었습니다.');
