...
const QnaAnswerList = () => {
  const [dataList, setDataList] = useState<QnaAnswerArticle[]>([]);
  ...
  const fetchQnaAnswerList = async () => {
    try {
      ...
      const response: any = await apiClient.get(`/user/help/qna/admin/selectQnaAnswerList.api?${params.toString()}`);
      if (response.resultCode === 'SUCCESS') {
        setDataList(response.resultList || []);
        setTotalCount(response.totalCount || 0);
      }
    } catch (error) {
      console.error('Q&A 답변 목록 조회 실패', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="list-utils">
        ...
        <div className="search-box">
          ...
          >
            ...
            <option value="qnaProcessSttusCodeNm">진행상태</option>
          </select>
          <input 
            type="text" 
            ...
            dataList.map((item, idx) => (
              <tr key={item.qaId}>
                ...
                <td style={{ textAlign: 'left' }}>
                  <Link to={`/admin/qna/${item.qaId}`}>{item.qestnSj}</Link>
                </td>
                <td>{item.wrterNm}</td>
